...
\begin{itemize}[parsep=0.5ex]
  \item 负责开发实现融合算子 AG+GEMM、AG+MoE、GEMM+AR 在 H800/L20/A800 上的融合算子
  \item 负责 GPU 跨机通信优化，基于多 QP 技术优化 All-to-All 算子在大规模 H20 集群上的性能
  \item GEMM+AR 算子端到端性能提升 $18\%$ (Prefill), $30\%$ (Decode)，预计接入 SGLang 
\end{itemize}

\datedsubsection{\textbf{基于 GPU 主导通信的集合通信库设计与实现}}{字节跳动-Seed (AI Infra)，北京}
\datedsubsection{\textit{主要开发者}}{2024.12-至今}
\begin{itemize}[parsep=0.5ex]
...
\datedsubsection{\textit{主要开发者}}{2024.10-2025.1}
\begin{itemize}[parsep=0.5ex]
  ...
  \item 集合通信算子已上线内部豆包大模型多模态推理模块
\end{itemize}

\datedsubsection{\textbf{计算通信融合算子自动生成}}{字节跳动-Seed (AI Infra)，北京}
\datedsubsection{\textit{主要开发者}}{2024.6-2025.3}
\begin{itemize}[parsep=0.5ex]
...
\begin{itemize}[parsep=0.5ex]
  ...
  \item 对不同集合通信算法下的物理节点和逻辑节点通信模式建模，分析通信拓扑、链路对任务训练时间的影响，设计任务部署算法降低光网络下跨机架任务通信量
\end{itemize}
...
\begin{itemize}[parsep=0.5ex]
  ...
  \item 基于可编程控制面设计并实现多后端故障注入系统，提供一系列参数供用户自定义流量协议
...
\begin{itemize}[parsep=0.5ex]
  ...
  \item 基于 Pytorch 实现 8 机 PS 架构分布式模型训练原型系统 (\href{https://github.com/Fangjin98/distributed-training-INA/}{\textit{链接}})
...
\begin{enumerate}[parsep=0.5ex]
   \item \textbf{J. Fang}, S. Zheng, X. Di, H. Gong, etc, \textit{Deploying Collective Kernels for Heterogeneous Links Using Tensor-Centric Primitives},  (\textbf{EuroSys'26, In Submission})
   \item S. Zheng, X. Zheng, S. Li, H. Sun, Q. Hou, \textbf{J. Fang}, etc, \textit{DITRON: A Flexible and Versatile Distributed Tensor Compiler for LLM},  (\textbf{ICLR'26, In Submission})
...
\begin{itemize}[parsep=0.5ex]
  \item \datedline{第一届 CCF ACNC 集合通信比赛一等奖（1/77）}{2025}
  \item \datedline{中国电科十四所国睿奖学金}{2023}
  \item \datedline{英特尔 P4 中国黑客松优胜奖}{2022}
  \item \datedline{一等学业奖学金（博士）$\times 2$}{2022, 2023, 2024}
